...
,@NamedQuery(name="Patient.findPatByName", query="SELECT p FROM Patient p where p.patFName=:patFName and p.patMName=:patMName and p.patLName=:patLName")
,@NamedQuery(name="Patient.findByPhone", query="SELECT p FROM Patient p where p.patPhone1=:patPhone1 or p.patPhone2=:patPhone1")
,@NamedQuery(name="Patient.findByEmail", query="SELECT p FROM Patient p where p.patEmail=:patEmail")})
