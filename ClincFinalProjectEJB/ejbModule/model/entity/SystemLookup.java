...
@NamedQueries({@NamedQuery(name="SystemLookup.findAll", query="SELECT s FROM SystemLookup s where s.lookupMajorId=:majorid and s.minorId IS NULL")
,@NamedQuery(name="SystemLookup.findAll2", query="SELECT s FROM SystemLookup s where s.minorId=:lookupId ")})
